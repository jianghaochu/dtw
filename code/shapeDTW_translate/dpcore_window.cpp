...
// [[Rcpp::export]]
List dpcore(NumericMatrix M, NumericMatrix C, Rcpp::Nullable<Rcpp::IntegerVector> ws = R_NilValue) // <<< --- Modified
{
	...
	for (j = 0; j < cols; ++j) {
		...
		/*Modified*/
		if (ws.isNotNull()) {
			int i_start = std::max(NumericVector::create(0, j-ws-1));
			int i_end = std::min(NumericVector::create(rows, j+ws-1));
		} else {
			int i_start = 0;
			int i_end = rows;
		}
